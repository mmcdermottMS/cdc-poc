...
namespace CDC.SbConsumer
{
    internal class CosmosDbService : ICosmosDbService
    {
        ...
        public async Task<TargetAddress> GetTargetAddressByProfileIdAsync(Guid profileId)
        {
            ...
            var query = new QueryDefinition(query: "SELECT * FROM addresses a WHERE a.partitionKey = @key").WithParameter("@key", profileId);
        ...
        public async Task UpsertTargetAddress(TargetAddress targetAddress)
        {
            ...
            await _container.CreateItemAsync<TargetAddress>(item: targetAddress, partitionKey: partitionKey);
        }
    }
}
